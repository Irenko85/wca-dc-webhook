...
def get_competitions(country: str = DEFAULT_COUNTRY) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info(f"Successfully fetched {len(competitions)} competitions from WCA API")
        return competitions
...
def detect_new_competitions(current_comps: List[Dict[str, Any]], 
                           previous_comps: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
...
def create_discord_embeds(competitions: List[Dict[str, Any]], 
                         is_new: bool = False) -> List[Dict[str, Any]]:
    ...
    for comp in competitions:
        ...
        start_date_fmt = datetime.datetime.strptime(start_date, "%Y-%m-%d").strftime("%d/%m/%Y")
        ...
        color = EMBED_COLORS["new"] if is_new else EMBED_COLORS.get(status, EMBED_COLORS["upcoming"])
        ...
        if comp.get("registration_open") and comp.get("registration_close"):
            reg_open = datetime.datetime.strptime(comp["registration_open"], "%Y-%m-%dT%H:%M:%S.%fZ").strftime("%d/%m/%Y")
            reg_close = datetime.datetime.strptime(comp["registration_close"], "%Y-%m-%dT%H:%M:%S.%fZ").strftime("%d/%m/%Y")
        ...
        events_text = ", ".join(event_names) if event_names else "No hay eventos disponibles"
...
def send_discord_notification(competitions: List[Dict[str, Any]], 
    ...
    for i in range(0, len(embeds), batch_size):
        ...
        try:
            ...
            response.raise_for_status()
            logger.info(f"Discord notification sent successfully ({len(batch_embeds)} competitions)")
        except requests.exceptions.RequestException as e:
...
def main() -> None:
    ...
    if not current_comps:
        logger.warning("No competitions found or error fetching data")
        return
        
    # Clean out competitions that have already ended
    removed_count = clean_old_competitions()
    
    # Load previous competitions (after cleaning)
    previous_comps = load_previous_competitions()
    
    logger.info(f"Comparing {len(current_comps)} current competitions with {len(previous_comps)} stored competitions")
